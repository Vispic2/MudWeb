...
var dirnw = '　', dirn = '　', dirne = '　', dirw = '　', dire = '　', dirsw = '　', dirso = '　', dirse = '　', tit = '一个江湖', titled = tit + '' + '';
...
var Wp1 = '4dgWt!hGUo'; //客户端key验证码
var user_key = '123456789abcd';
var chat_height = 0;
...
function initBT() {
	...
	for (var i = 1; i < 11; i++) {
		hudongob.append('<button style="color:' + textcol + ';width:19%;height:40px;" type="button" id="mybt' + i + '" onclick="setBT(' + i + ')">自定' + i + '</button>');
	}
}
...
function writechar() {
	...
	hudongob.append('<form class="Radio">    <input id="nan" name="sex" type="radio" checked value="男性"/><label for="nan">男</label>    <input id="nv" name="sex" type="radio" value="女性" /><label for="nv">女性</label> </form><br>');
};
...
function registercheck() {
	...
	console.log(getDateYMD() + Wp1 + myid + mypass + myphone + mymail)
...
function config() {
	...
	hudongob.append('<input type="button" style="color:' + textcol + ';margin:3px;width:145px;height:35px" id="toggleBtn" value=' + long_value + ' onclick="room_set()" />');
	hudongob.append('<input type="button" style="color:' + textcol + ';margin:3px;width:145px;height:35px" id="toggleChatBtn" value=' + chat_value + ' onclick="toggleChat()" />');
	//hudongob.append('<br><input type="button" style="color:#ff0000;margin:3px;width:145px;height:35px" id="exit_login" value="游戏充值" onclick="paym()" />');
	hudongob.append('<input type="button" style="color:' + textcol + ';margin:3px;width:145px;height:35px" id="exit_login" value="退出游戏" onclick="relogin()" />');
	hudongob.css('height', 'auto');
...
function writeServerData(buf) {
	...
	if (line.substr(0, 8) == '0000007') {
		...
		menuob.html('<table width="100%" border="0" cellspacing="0" cellpadding="1"><tr>'
			...
			+ '<td align="center" valign="middle"><input class="menubt" type="button" name="bcmd" id="cmd" onclick="javascript:userBT()" value="自定"></td>'
...
$(document).ready(function () {
	...
	sock.on('stream', function (buf) {
		...
		for (var ii = 0; ii < tmpstr.length; ii++) {
			if (buf.charAt(buf.length - 1) != "\n" && ii == (tmpstr.length - 1) && tmpstr.indexOf("006") == 0) {
				strsss += tmpstr[ii];
				return;
			}
